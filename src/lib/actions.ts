...
export async function addServer(serverData: unknown) {
  ...
  try {
    ...
    await db.collection("servers").insertOne(serverToInsert);
...
export async function handleSupportRequest(
    ...
    <div style="font-family: Arial, 'Helvetica Neue', Helvetica, sans-serif; line-height: 1.6; color: #333;">
      <div style="max-width: 600px; margin: 20px auto; padding: 25px; border: 1px solid #e0e0e0; border-radius: 10px; box-shadow: 0 4px 8px rgba(0,0,0,0.05);">
        <h2 style="color: #1a1a1a; border-bottom: 2px solid #f0f0f0; padding-bottom: 15px; margin-top: 0;">New Support Request from Remote Commander</h2>
        <p style="font-size: 16px;">You've received a new support request with the following details:</p>
        <div style="background-color: #f9f9f9; padding: 20px; border-left: 4px solid #007bff; border-radius: 5px; margin-top: 20px;">
          <p style="margin: 0 0 10px;"><strong>Name:</strong> ${name}</p>
          <p style="margin: 0;"><strong>Email:</strong> <a href="mailto:${email}" style="color: #007bff; text-decoration: none;">${email}</a></p>
        </div>
        <div style="margin-top: 25px;">
          <h3 style="color: #1a1a1a; font-size: 18px; margin-bottom: 10px;">Message:</h3>
          <p style="white-space: pre-wrap; word-wrap: break-word; background-color: #f9f9f9; padding: 20px; border-radius: 5px; margin-top: 0; font-size: 15px;">${message}</p>
        </div>
        <p style="font-size: 12px; color: #999; text-align: center; margin-top: 30px;">This email was automatically generated by the Remote Commander application.</p>
      </div>
    </div>
  `;
    ...
    try {
        await transporter.sendMail({
            from: `"${name}" <${SMTP_USER}>`, // Send from your authenticated user
            replyTo: `"${name}" <${email}>`, // Set the user's email as the reply-to address
